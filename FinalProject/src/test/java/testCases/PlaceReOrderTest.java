...
public class PlaceReOrderTest extends BuildUp{
	...
	@Test
	public void verifyReOrderPlaced() throws InterruptedException
	{
		...
		mop.getReOrder().click();

		
		CartPageObjects cp = new CartPageObjects(driver);
	//	cp.getSonyQty().click();
		cp.getSonyQty().sendKeys("3");
		cp.getSonyQtyBtn().click();
		String tot=cp.getGrandTotal().getText();
		Assert.assertEquals(tot, "$7,995.00");
